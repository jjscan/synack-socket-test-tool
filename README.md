...
[**Releases**](https://github.com/jjscan/synack-socket-test-tool/releases/latest) 에서 `SYNACK_Setup_v2.2.0.exe` 를 받아 실행하면 됩니다.
...
이 표는 **클라이언트에서도 똑같이** 쓸 수 있습니다. 아래 '클라이언트 자동 응답'을 보세요.

### 클라이언트 자동 응답 — 상대가 보내오면 회신하기

클라이언트도 **받으면 자동으로 회신**할 수 있습니다. 서버가 먼저 말을 거는 장비를 흉내낼 때 씁니다.

연결 추가 창의 **`자동 응답 Auto Reply`** 에서 고릅니다.

| 고르는 것 | 동작 |
| --- | --- |
| **수동 응답** (기본값) | 받기만 하고 회신하지 않음 |
| **수신 후 응답** | 무엇을 받든 정해진 값으로 회신 (`지속 응답`을 끄면 처음 한 번만) |

여기에 **규칙 기반 응답**을 더하면 받은 내용에 따라 다르게 회신합니다. 규칙은 `수동 응답`을 골라도 동작하며, 고정 응답보다 우선합니다.

**주기 전송과 같이 켜도 됩니다.** 주기적으로 하트비트를 보내면서, 서버가 무언가 보내면 ACK로 답하는 실제 장비 동작을 그대로 흉내낼 수 있습니다.

#### 먼저 말을 걸어야 할 때 — 접속 인사

양쪽 다 "받으면 응답"으로 두면 **아무도 먼저 말하지 않아 영영 대기합니다.** 누군가는 첫 마디를 던져야 합니다.

클라이언트에서 **`접속하면 먼저 보내기 Send on Connect`** 를 켜고 보낼 데이터를 넣으면, 서버에 붙자마자 그 내용을 한 번 보냅니다. 자동 응답과 **따로** 동작하므로 둘을 같이 켤 수 있습니다.

```
접속 → LOGIN 전송 → 서버 응답 → 클라이언트가 ACK 회신 → ...
```

첫 마디를 던지는 방법은 이것 말고도 **수동 `1회 전송`** 과 **`주기 전송`** 이 있습니다. 상대 서버가 `접속 시 1회 전송`이면 서버가 먼저 말을 걸어 줍니다.

> 클라이언트에는 **Echo**가 없습니다. 상대 서버도 Echo이면 둘이 끝없이 주고받게 되기 때문입니다.

#### 쪼개져 오는 전문 합치기

`자동 응답`이나 규칙을 쓰면 **`수신 대기 Receive timeout`** 입력란이 나타납니다. 그 시간만큼 더 들어오는 데이터가 없으면 한 건으로 합쳐서 판정합니다.

전문이 두 조각으로 나뉘어 도착해도 규칙이 제대로 걸리게 하려면 이 값이 필요합니다. **`0`이면 합치지 않고 받는 대로** 처리합니다.
...
| [qa/README.md](qa/README.md) | 검증 하네스 실행 방법 (기능 225개 / 부하 60개) |
